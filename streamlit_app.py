...
def load_data():
    ...
    # 1. Komma-Korrektur (confidence_score)
    if 'confidence_score' in df.columns:
        df['confidence_score'] = df['confidence_score'].str.replace(',', '.').pipe(pd.to_numeric, errors='coerce')

    # 2. INTELLIGENTE EXTRAKTION DER LEVELS
    # Wir ziehen die erste Zahl aus den Spalten (Extrahiert '1' aus '1st-line' oder '1st level')
...
try:
    ...
    with c3:
        # Filter nutzt die Originalnamen aus der Tabelle
    ...
    # Hier nutzen wir die extrahierten Zahlen für den Vergleich
    if 'role_num' in data.columns and 'level_num' in data.columns:
        ...
        kpi3.metric("Routing Accuracy", f"{acc:.1%}", help="Match between Predicted Level and Owner Role number")
    ...
    with col_a:
        ...
        monthly = data.groupby('month').size().reset_index(name='Tickets')
        st.plotly_chart(px.line(monthly, x='month', y='Tickets', title="Ticket Volume per Month", markers=True), use_container_width=True)

    with col_b:
        st.plotly_chart(px.pie(data, names='predicted_level', title="Level Distribution", hole=0.4), use_container_width=True)

    # Heatmap mit den Original-Labels für bessere Lesbarkeit
